...
uint8_t& MemoryBus::readByte(uint16_t address) {
    if (address <= 0x3FFF) { // 0000–3FFF: ROM Bank 0
        return singleton.getROM()[address];
    } else if (address >= 0x4000 && address <= 0x7FFF) { // 4000–7FFF: ROM Bank N
        return singleton.getROM()[address];
    } else if (address >= 0x8000 && address <= 0x9FFF) { // 8000–9FFF: VRAM
        return singleton.getVRAM()[address - 0x8000];
    } else if (address >= 0xA000 && address <= 0xBFFF) { // A000–BFFF: External RAM
        return singleton.getRAM()[address - 0xA000];
    } else if (address >= 0xC000 && address <= 0xCFFF) { // C000–CFFF: WRAM Bank 0
        return singleton.getWRAMBank0()[address - 0xC000];
    } else if (address >= 0xD000 && address <= 0xDFFF) { // D000–DFFF: WRAM Bank 1
        return singleton.getWRAMBank1()[address - 0xD000];
    } else if (address >= 0xE000 && address <= 0xFDFF) { // E000–FDFF: Echo RAM
        return singleton.getEchoRAM()[address - 0xE000];
    } else if (address >= 0xFE00 && address <= 0xFE9F) { // FE00–FE9F: OAM
        return singleton.getOAM()[address - 0xFE00];
    } else if (address >= 0xFEA0 && address <= 0xFEFF) { // FEA0–FEFF: Unusable
        throw std::out_of_range("Unusable memory region");
    } else if (address >= 0xFF00 && address <= 0xFF7F) { // FF00–FF7F: IO Registers
        return singleton.getIORegisters()[address - 0xFF00];
    } else if (address >= 0xFF80 && address <= 0xFFFE) { // FF80–FFFE: HRAM
        return singleton.getHRAM()[address - 0xFF80];
    } else if (address == 0xFFFF) { // FFFF: Interrupt Enable
...
uint8_t& writeByte(uint16_t address, uint8_t value) {
    if (address <= 0x3FFF) { // 0000–3FFF: ROM Bank 0
        std::cout << "Attempting to write to ROM Bank 0 at address " << std::hex << address << std::endl;
        return singleton.getROMBank0()[address];
    } else if (address >= 0x4000 && address <= 0x7FFF) { // 4000–7FFF: ROM Bank N
        std::cout << "Attempting to write to ROM at address " << std::hex << address << std::endl;
...
uint8_t& writeVram(uint16_t address, uint8_t index_value, uint8_t value) {
    ...
    value = switch(bit1, bit2){
        case (0,0): return 0; //white
        case (0,1): return 1; //light gray
        case (1,0): return 2; //dark gray
        case (1,1): return 3; //black
    };
};
